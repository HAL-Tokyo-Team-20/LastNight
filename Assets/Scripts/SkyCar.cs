...
public class SkyCar : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            transform.DOMoveX(transform.position.x - DeadEnd, DeadEnd / MoveSpeed).SetEase(Ease.Linear).OnComplete(() =>
               {
        ...
        {
            transform.DOMoveZ(transform.position.z - DeadEnd, DeadEnd / MoveSpeed).SetEase(Ease.Linear).OnComplete(() =>
               {
                   GameObject.Destroy(this.gameObject);
               });
        }
